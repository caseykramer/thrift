...
namespace ThriftMSBuildTask
{
    ...
    public class ThriftBuild : Task
    {
        ...
        public ITaskItem GenDir { get; set; }

        /// <summary>
        /// References to pass on to csc
        /// </summary>
        public ITaskItem[] References { get; set; }

        /// <summary>
        /// AdditionalLibPaths for csc
        /// </summary>
        public ITaskItem[] AdditionalLibPaths { get; set; }

        /// <summary>
        /// KeyFile to pass on to csc
        /// </summary>
        public ITaskItem KeyFile { get; set; }

        /// <summary>
        /// Warning level to set on csc
        /// </summary>
        public ITaskItem CSCWarningLevel { get; set; }

        public ITaskItem ExcludePattern { get; set; }
        ...
        public override bool Execute()
        {
            ...
            if (!String.IsNullOrEmpty(GenDir.ItemSpec))
            {
                thriftDir = SafePath(GenDir.ItemSpec);
            }
            ...
            var allRefs = new List<ITaskItem>(this.References);
            allRefs.Add(new TaskItem(ThriftLibrary.ItemSpec));
            csc.References = allRefs.ToArray();

            csc.EmitDebugInformation = true;
            string outputPath = SafePath(OutputName.ItemSpec);
            csc.NoConfig = true;
            csc.NoStandardLib = true;
            csc.AdditionalLibPaths = this.AdditionalLibPaths.Select(ti => ti.ItemSpec).ToArray();
            csc.DebugType = "pdbonly";
            csc.OutputAssembly = new TaskItem(outputPath);
            var exclusions = new HashSet<string>();
            if (ExcludePattern != null && !String.IsNullOrEmpty(ExcludePattern.ItemSpec))
            {
                foreach (var xcl in FindSources(SafePath(ExcludePattern.ItemSpec)))
                {
                    exclusions.Add(xcl.ItemSpec);
                }
                
            }
            csc.Sources = FindSources(Path.Combine(thriftDir, "gen-csharp")).Where(item => !exclusions.Contains(item.ItemSpec)).ToArray();
            csc.BuildEngine = this.BuildEngine;
            if (!string.IsNullOrEmpty(this.CSCWarningLevel.ItemSpec))
            {
                var level = 1;
                int.TryParse(this.CSCWarningLevel.ItemSpec, out level);
                csc.WarningLevel = level;
            }

            csc.KeyFile = SafePath(this.KeyFile.ItemSpec);
